...
export const GalleryWall: React.FC<GalleryWallProps> = ({ currentArt, state }) => {
  ...
  // The loading state is true if we are generating OR if the image exists but hasn't finished loading yet
  const isVisuallyLoading = state === GeneratorState.GENERATING || (currentArt !== null && !imageLoaded);

  return (
    <div className="flex-1 flex items-center justify-center p-4 md:p-8 bg-neutral-900 relative overflow-hidden min-h-[60vh]">
      ...
      {/* Frame Container */}
      <div className="relative z-10 max-w-lg w-full aspect-[3/4] transition-transform duration-700 ease-out">
              ...
              <div className="w-full h-full bg-[#e0e0e0] relative overflow-hidden flex items-center justify-center shadow-inner">
                ...
                {currentArt && (
                  <img 
                    ...
                    className={`w-full h-full object-cover transition-all duration-1000 ease-in-out ${isVisuallyLoading ? 'opacity-0 scale-105' : 'opacity-100 scale-100 grayscale-[0.1]'}`}
                  />
                )}
                ...
                {/* Loading State - Breathing Mist (Not a spinner) */}
                {isVisuallyLoading && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#e0e0e0] z-20 transition-opacity duration-500">
                     {/* The Breathing Orb */}
